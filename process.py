...
from pdf_generator import get_template, gen_pdf
    ...
    for filename in os.listdir(dir):
        ...
        if os.path.isfile(f):
            ...
            if (not regen) and os.path.isfile(target_file):
                ...
                body = read(f).strip()
                gen_pdf(body, config, outsubdir, filename, target_file)
        if os.path.isdir(f):
...
def generate_index(path, relative_dir="/", parent=""):
    ...
    directories.sort()
    dir_rows = [
        "<tr><td><span class=\"icon-text\"><span class=\"icon\"><icon class=\"fas fa-regular fa-folder\"></icon></span><span><a href=\"./" + d + "/index.html\">" + d + "</a></span></span></td></tr>"
        for d in directories]
    file_rows = [
        "<tr><td><span class=\"icon-text\"><span class=\"icon\"><icon class=\"fas fa-regular fa-file-pdf\"></icon></span><span><a href=\"./" + f + "\">" + f + "</a></span></span></td></tr>"
        for f in files if f.endswith(".pdf")]
    template = get_template("index_page.html")
    file_blob = "\n".join(file_rows)
    dir_blob = "\n".join(dir_rows)
